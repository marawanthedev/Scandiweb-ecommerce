...
import { addAttributeSelectionsIndex } from "../../redux/cart/cart.actions"
...
class ProductDisplayPage extends React.PureComponent {
    ...
    handleAttributeSelection = (newAttributeIndex, newAttributeSelectionIndex) => {
        ...
        this.setState({ attributeIndex: attributeIndex, attributeSelectionIndex: attributeSelectionIndex })
    ...
    handleAddToCartAction = (item, attributeIndex, attributeSelectionIndex,) => {
        ...
        if (this.confirmAttributeSelections(item.attributes, attributeIndex, attributeSelectionIndex)) {
            ...
            setTimeout(() => this.setState({ showAttributeSelectionAlert: false }), 2500)
        }
    }
        ...
        return (
            <div className="productDisplayPage">
                {showAttributeSelectionAlert ?
                    <Zoom><div className="productDisplayPage__alert-box">
                        <div className="productDisplayPage__alert-box__closeIcon" onClick={this.closeAlertPopUp} style={{ backgroundImage: `url(${CloseIcon})` }}>
                        </div>
                        <div className="productDisplayPage__alert-box__warningIcon" style={{ backgroundImage: `url(${WarningIcon})` }}>
                        </div>
                ...
                <div className="productDisplayPage__left-side">
                    <div className="productDisplayPage__left-side__images-gallery">
                        {item.gallery.map((galleryItem, index) =>
                            <div className="productDisplayPage__left-side__images-gallery__item" key={index} onClick={() => this.setState({ selectedImageIndex: index })} style={{ backgroundImage: `url(${galleryItem})` }}>
                            </div>
                        )}
                    </div>
                    <div className="productDisplayPage__left-side__selected-image-container" style={{ backgroundImage: `url(${item.gallery[selectedImageIndex]})` }}>
                    </div>
                </div>
                    ...
                    <div className="productDisplayPage__product__info__item-attributes__container">
                        {
                            item.attributes.map((attribute, index) => (
                                <ItemAttribues key={index} attribute={attribute} attributeIndex={index}
                                    attributeSelectionIndex={attributeSelectionIndex[attributeSelectionIndex.length - 1]} toggleButtons={true} item={item} onClickCallBack={this.handleAttributeSelection}>
                                </ItemAttribues>))
                        }
                    </div>
